		...
		success: function(result) {
			...
			for (let i = 0; i < 3; i++) {
                ...
                const name = 
                    ...
                    ${randomThreeEmployees[i]['last_name']}`.toUpperCase()
...
function getTitles(employeeArray) {
	$.ajax({
		...
		success: function(result) {
			const employeeTitles = result.employees;
			for (let i = 0; i < employeeArray.length; i++) {
				const title = employeeTitles[employeeArray[i]["id"] - 1]["title"].toLowerCase();
...
function toggleMobileMenu() {
	if ($('.mobile-menu').hasClass('fa-bars')) {
		$('.nav-links ul').css('animation', 'open .5s both');
		$('.nav-links').css('margin-bottom', '-16px');
		$('.mobile-menu').addClass('fa-times').removeClass('fa-bars').css('color', teal);
		$('.banner').css('display', 'none');
	} else {
		$('.nav-links ul').css('animation', 'collapse .5s both')
